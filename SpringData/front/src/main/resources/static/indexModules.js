angular.module('app', ['ngStorage']).controller('indexController', function ($scope, $http, $localStorage, $window) {
    ...
    $scope.tryToAuth = function () {
        $http.post('http://localhost:8250/auth', $scope.user)
...
//        $http.get('http://localhost:8100/auth').then(function (response) {
...
//    };
    ...
    $scope.loadProducts = function () {
        $http.get('http://localhost:8050/products/v1').then(function (response) {
            $scope.productsList = response.data;
        });
    }

    $scope.addToCart = function (productId) {
        $http.get('http://localhost:8150/cart/v1/add/' + productId).then(function (response) {
            $scope.loadCart();
        });
    }

    $scope.loadCart = function () {
        $http.get('http://localhost:8150/cart/v1').then(function (response) {
            $scope.cart = response.data;
        });
    }

    $scope.eraseCart = function () {
        $http.delete('http://localhost:8150/cart/v1/eraseCart').then(function (response) {
            $scope.loadCart();
        });
    }

    $scope.deleteCartItem = function (productId) {
        $http.get('http://localhost:8150/cart/v1/deleteCartItem/'+ productId).then(function (response) {
            $scope.loadCart();
        });
    }

   $scope.addQuantity = function (productId, n) {
        $http.get('http://localhost:8150/cart/v1/addQuantity/' + productId + '?quantity=' + n).then(function (response) {
            $scope.loadCart();
        });
   }

    $scope.createOrder = function () {
        $http.get('http://localhost:8050/orders/v1/createOrder').then(function (response) {
            $scope.loadCart();
        });
   }
...
//                                  "&maxCost="+$scope.sort.maxCost+"&title="+$scope.sort.title);
        ...
        $http.get('http://localhost:8050/products/v1?minCost='+ $scope.sort.minCost +
