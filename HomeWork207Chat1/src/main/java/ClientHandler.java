...
public class ClientHandler {
            ...
            if (str.startsWith("/auth")) {
                ...
                if (nick != null) {
                    if (!myServer.isNickBusy(nick)) {
                        sendMsg("/authok " + nick);
                        name = nick;
                        myServer.broadcastMsg(name + " зашел в чат", "all");
    ...
    public void readMessages() throws IOException {
        while (true) {
            ...
            if (strFromClient.startsWith("/w")) {
                ...
                myServer.broadcastMsg(name + ": " + strFromClient,privatTextNick[1]);
                continue;
            }

            myServer.broadcastMsg(name + ": " + strFromClient,"all");
        }
    }
    ...
    public void closeConnection() {
        ...
        myServer.broadcastMsg(name + " вышел из чата", "all");
        try {
            in.close();
        } catch (IOException e) {
            e.printStackTrace();
